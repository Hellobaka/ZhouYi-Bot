...
using me.cqp.luohuaming.ZhouYi.Core;
using me.cqp.luohuaming.ZhouYi.Core.Domain;
using me.cqp.luohuaming.ZhouYi.Sdk.Cqp;
using me.cqp.luohuaming.ZhouYi.Sdk.Cqp.Enum;
using me.cqp.luohuaming.ZhouYi.Sdk.Cqp.EventArgs;
using me.cqp.luohuaming.ZhouYi.Sdk.Cqp.Interface;
using me.cqp.luohuaming.ZhouYi.Sdk.Cqp.Expand;
using me.cqp.luohuaming.ZhouYi.Sdk.Cqp.Model;
using Unity;
using Unity.Injection;

namespace me.cqp.luohuaming.ZhouYi.App.Export
{
	/// <summary>	
	/// 表示酷Q事件导出的类	
	/// </summary>	
	...
	{	
		...
		{	
			return "9,me.cqp.luohuaming.ZhouYi";	
		}	
		...
		{	
			...
			Type appDataType = typeof (AppData);	
			// 注册一个 CQApi 实例	
			AppInfo appInfo = new AppInfo ("me.cqp.luohuaming.ZhouYi", 1, 9, "水银周易机", "1.0.0", 1, "落花茗", "定制插件 2021.9.14", authCode);	
			appDataType.GetRuntimeProperty ("CQApi").GetSetMethod (true).Invoke (null, new object[] { new CQApi (appInfo) });	
			AppData.UnityContainer.RegisterInstance<CQApi> ("me.cqp.luohuaming.ZhouYi", AppData.CQApi);	
			// 向容器注册一个 CQLog 实例	
			appDataType.GetRuntimeProperty ("CQLog").GetSetMethod (true).Invoke (null, new object[] { new CQLog (authCode) });	
			AppData.UnityContainer.RegisterInstance<CQLog> ("me.cqp.luohuaming.ZhouYi", AppData.CQLog);	
